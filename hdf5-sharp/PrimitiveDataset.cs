...
//        }
        ...
        // FIXME: this throws if T is not a value type.
        public void Write(Dataspace ms, Dataspace fs, Array data)
        {
            if (data.GetType().GetElementType() == null)
...
//                    hmdata[i] = null;
        ...
        public T this[int i]
        {
            get { return this[(ulong)i]; }
            set { this[(ulong)i] = value; }
        }
        ...
        public T this[long i]
        {
            get
            {
                ...
                if (dim.Length != 1)
                    throw new InvalidOperationException(String.Format("Cannot access {0}-dimensional string with one index.", dim.Length));
                T[] result = new T[1];
                using (Datatype mt = Datatype.FromValueType(typeof(T)))
                using (Dataspace ms = new Dataspace(new long[] {1}))
                using (Dataspace fs = new Dataspace(dim))
                {
                    // select i-th element
                    fs.SelectHyperslab(SelectOperation.Set, new long[] {i}, null, new long[] {1}, null);
                    // pin and read
            ...
            set
            {
                ...
                {
                    // select i-th element
                    fs.SelectHyperslab(SelectOperation.Set, new ulong[] {i}, null, new ulong[] {1}, null);
                    // pin and read
                    GCHandle hres = GCHandle.Alloc(value, GCHandleType.Pinned);
                    try {
                        Read(mt, ms, Dataspace.All, hres.AddrOfPinnedObject());
                    } finally {
                        hres.Free();
        ...
        public static Dataset<TT> Create<TT>(Location loc, string name, int[] space)
        {
            ulong[] lspace = new ulong[space.Length];
            for (int i=0; i<space.Length; i++)
                lspace[i] = (ulong)space[i];
            return Create<TT>(loc, name, lspace);
        }
        
        public static Dataset<TT> Create<TT>(Location loc, string name, ulong[] space)
        {
            return Create<TT>(loc, name, ByteOrder.LittleEndian, space);
        }
        
        public static Dataset<TT> Create<TT>(Location loc, string name, ByteOrder order, ulong[] space)
        {
            Dataset result = null;
            using (Datatype dt = Datatype.FromValueType(type, order))
            using (Dataspace ds = new Dataspace(space))
                result = Create(loc, name, dt, ds);
            return result;
        }
        
        private static Dataset<TT> Create<TT>(Location loc, string name, Datatype type, Dataspace space)
        {
            return new Dataset<TT>(H5Dcreate2(loc.raw, name, type.raw, space.raw, 0, 0, 0));
        }
        
        public static Dataset<TT> CreateWithData<TT>(Location loc, string name, ByteOrder order, TT data) where TT : struct
        {
            Dataset result = null;
            using (Datatype dt = Datatype.FromValueType(typeof(TT), order))
            {
        ...
        public static Dataset<TT> CreateWithData<TT>(Location loc, string name, TT data) where TT : struct
        {
            return Dataset.CreateWithData<TT>(loc, name, ByteOrder.LittleEndian, data);
        }
        
        public static Dataset<BitArray> CreateWithData(Location loc, string name, ByteOrder order, BitArray data)
        {
            Dataset result = null;
        ...
        public static Dataset<BitArray> CreateWithData(Location loc, string name, BitArray data)
        {
        ...
        public static Dataset<string> CreateWithData(Location loc, string name, string data)
        {
            Dataset result = null;
        ...
        public static Dataset<TT> Open<TT>(Location loc, string name)
        {
